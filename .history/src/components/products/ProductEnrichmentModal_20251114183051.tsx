...
export default function ProductEnrichmentModal({
  ...
  const fetchWebExtraction = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/products/extract-web-metadata', {
        ...
        body: JSON.stringify({
          productName: productName || selectedMethod?.data?.name,
          productEan: productEan || selectedMethod?.data?.ean,
          productBrand: selectedMethod?.data?.brand,
          tenantId: user.tenantId,
        }),
      });
      ...
      const data = await response.json();
      const sanitizedImages = sanitizeImageUrls(data.selectedMethod?.data.images);

      setSuggestions(prev => {
        const nextSuggestions = { ...prev, ...data.selectedMethod?.data } as EnrichmentSuggestion;

        if (data.selectedMethod?.data.images) {
      ...
      setDescriptionsByMethod(prev => ({
        ...prev,
        web_extraction: data.selectedMethod?.data.description || prev.web_extraction,
      }));

      // Re-initialize approvals
      const initialApprovals: FieldApproval = {};
      Object.keys(data.selectedMethod?.data).forEach(key => {
        if (key !== 'images') {
  ...
  const fetchGroundedEnrichment = async () => {
    ...
    try {
      ...
      setDescriptionsByMethod(prev => ({
        ...prev,
        grounded: data.selectedMethod?.data.description || prev.grounded,
      }));
